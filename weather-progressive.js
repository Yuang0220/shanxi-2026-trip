(() => {
  ...
  function fmtNow(){
    return new Date().toLocaleString('zh-CN',{timeZone:TZ,month:'numeric',day:'numeric',hour:'2-digit',minute:'2-digit',hour12:false});
  }
  function setStatus(text){
    const u=document.querySelector('#weather .updated');
  ...
  async function run(){
    ...
    try{
      ...
      if(!r.ok){ setStatus(`天气检查 ${fmtNow()} · 天气源暂不可用，显示远期参考`); return; }
      const data=await r.json(); const arr=Array.isArray(data)?data:[data];
      const byKey={};
      keys.forEach((k,idx)=>{
        const d=arr[idx]?.daily; if(!d) return; byKey[k]={};
        d.time.forEach((date,i)=>byKey[k][date]={code:d.weather_code[i],max:d.temperature_2m_max[i],min:d.temperature_2m_min[i],p:d.precipitation_probability_max[i],w:d.wind_speed_10m_max[i]});
      });
      let changed=false; const rs=rows();
      days.forEach(([date,key],i)=>{
        const f=byKey[key]?.[date]; if(!f||!rs[i]) return;
        const [,wx,temp,pw]=rs[i];
        wx.textContent=WMO[Math.round(f.code)]||'逐日预报'; wx.classList.remove('muted');
        temp.textContent=`${Math.round(f.min)}–${Math.round(f.max)}℃`;
        pw.textContent=`降水 ${Math.round(f.p ?? 0)}% · 风 ${Math.round(f.w ?? 0)} km/h`; pw.classList.remove('muted');
        changed=true;
      });
      if(changed) setStatus(`逐日预报更新 ${fmtNow()}`);
      else setStatus(`天气检查 ${fmtNow()} · 行程日期尚未进入逐日预报窗口`);
    }catch(e){
      setStatus(`天气检查 ${fmtNow()} · 天气源暂不可用，显示远期参考`);
    }
  }
